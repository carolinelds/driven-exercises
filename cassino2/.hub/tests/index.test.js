...
describe("Exercício cassino", () => {
  it("deveria imprimir o número da sorte com o número colorido", async () => {
    ...
    expectFormat(message.message, /\[BLUE\][0-9]+\[\/BLUE\]/, "Esperava que o número da sorte estivesse azul!");
  }, 15000);
  ...
  it("deveria imprimir 'você ganhou/perdeu...'", async () => {
    const result = await runTimedProcess("node .tested-file.cjs", 8000);
    start = result[0].time;
    const message = getMessageAlike(result, "ganhou") || getMessageAlike(result, "perdeu");

    expectMessage(message, "você ganhou/perdeu");
    expectCloseTime(message.time, 5000);

    if (message.message.toLowerCase().indexOf("ganhou") > -1) {
      expectFormat(message.message, /\[GREEN\].*\[\/GREEN\]/, `Esperava "Você ganhou!" verde!`);
    } else {
      expectFormat(message.message, /\[RED\].*\[\/RED\]/, `Esperava "Você perdeu" vermelho!`);
    }
  }, 15000);
});
